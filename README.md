...
- expect ma defaultowo 5_000 ms timeout, mozna to nadpisac w pliku konfiguracyjnym: config = { expect: { timeout: 1_000 } }
...
- jak zamienimy `test` na `test.only` to komenda `npx playwright test` uruchomi nam tylko ten jeden test. Mozemy to dodac do wiekszej ilosci testow
- metoda `fill()` ma juz wbudowany click, wiec nie trzeba klikac w element przed wpisywaniem wartosci
...
# GDZIE SKONCZYLEM
https://jaktestowac.pl/lesson/pw1s01l03/ - 34:24